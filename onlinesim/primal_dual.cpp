...
/*-------------------------------------------------------------------------------
primal_dual_ordering:
input args: set of coflows, the bigswitch over which coflows need to be scheduled
output: primal dual ordering returned by the algorithm
---------------------------------------------------------------------------------*/
std::vector<Coflow> primal_dual_ordering(std::vector<Coflow> coflows, Bigswitch bigs) {
  ...
  struct coflow *J = (struct coflow*) malloc(numCoflows * sizeof(struct coflow));   
  //allocate memory for array of coflows
  ...
  for (i = 0; i < numCoflows; i++) {
    ...
    J[i].scheduled = -1;
    J[i].D_k = (int**) malloc(B.portsize * sizeof(int*));     
    //allocate memory for 2D array D_k of size m*m
    int j;
    ...
    for(k=0;k<B.portsize;k++){
      for(l=0;l<B.portsize;l++){    
        //input the flow sizes in the newly allocated D_k matrix
        J[i].D_k[k][l] = 0;
      }
    }
    int f;
    ...
    J[i].v_k = (int*)malloc(2*B.portsize*sizeof(int));   
    //v_k is a vector of size #ports which hold total inflow/outflow for that port
    ...
    for(j=0;j<2*B.portsize;j++){
      ...
      if(j<=B.portsize-1){
        ...
        J[i].v_k[j] = 0;
        for(k=0;k<B.portsize;k++){
          J[i].v_k[j] += J[i].D_k[k][j-B.portsize];  
          //check the index
        }
      }
    ...
    int max=-1;    
    //Now find the port with maximum flow input/output
    for(j=0;j<2*B.portsize;j++){
      if(J[i].v_k[j]>max){
        max = J[i].v_k[j];
        J[i].m_k = j;
  ...
  int k=0,l=0;
  float* beta = (float*)malloc(numCoflows*sizeof(float));  
  //initialize beta array for the algorithm
  for(k=0;k<numCoflows;k++)
    beta[k] = 0;


  int *p_k = (int*)malloc(numCoflows*sizeof(int));  
  //initialize p_k for the algorithm
  int *temp_v_k = (int*)malloc(2*B.portsize*sizeof(int));


  for(k=1;k<=numCoflows;k++){    
    ...
    int min_j;
    int j;
    for(j=0;j<numCoflows;j++){  
      //we need to find the min value of term over all the jobs/coflows
      if(J[j].scheduled!=-1)    
      //if coflow is already scheduled, we do not consider the coflow
        continue;
      int n;
    ...
    beta[k-1] = min_beta;
    J[min_j].scheduled = numCoflows - k + 1;   
    //note that the final numbering goes from 1 to numCoflows
  }
  // int p;
